...
char *
get_new_charset(char *namebuf, const char *old_name)
{
  int len, i, j;
  char *p, *q;
  const struct alias *ap_env, *ap_old;
  ...
  for (i = 0; i < len; i++) {
    ...
    if (namebuf[i] == '=') {
      ...
      for (j = i + 1; j <= len; j++) {
        ...
        if (namebuf[j] == ':' || namebuf[j] == '\0') {
          namebuf[j] = '\0';
          if (!strcmp(p,old_name)) {
            return q;
          } else {
            ap_old = aliases_lookup(old_name,strlen(old_name));
            if (ap_old) {
              ap_env = aliases_lookup(p, strlen(p));
              if (ap_env && ap_old->encoding_index == ap_env->encoding_index) {
                return q;
              }
            }
          }
...
const struct alias *
aliases_lookup_with_env (register const char *str, register unsigned int len)
{
  char *charset_alias;
  char *namebuf;
  char *new_name;
  ...
  namebuf = malloc(strlen(charset_alias));
  if (!namebuf) {
    errno = ENOMEM;
    return (const struct alias *)NULL;
  }
  strcpy(namebuf, charset_alias);

  new_name = get_new_charset(namebuf, str);
  if (new_name) {
    ap = aliases_lookup(new_name, strlen(new_name));
  }
...
iconv_t iconv_open (const char* tocode, const char* fromcode)
{
  ...
  for (to_wchar = 0;;) {
    ...
    ap = aliases_lookup_with_env(buf,bp-buf);
    if (ap == NULL && errno == ENOMEM) {
      return (iconv_t)(-1);
    }
    if (ap == NULL) {
